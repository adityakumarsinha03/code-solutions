...
class Solution
{
    public static boolean ancestorsUtil(Node root, int target, ArrayList<Integer> ans){
        if(root == null)
            return false;
        ...
        if(ancestorsUtil(root.left, target, ans) || ancestorsUtil(root.right, target, ans)){
            ans.add(root.data);
            return true;
        } else
            return false;
